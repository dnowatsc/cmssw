...
class InclusiveVertexFinderMod : public edm::EDProducer {
    public:
	InclusiveVertexFinderMod(const edm::ParameterSet &params);

	virtual void produce(edm::Event &event, const edm::EventSetup &es);

    private:
	bool trackFilter(const reco::TrackRef &track) const;
	bool trackFromPrimVert(const reco::TrackRef &track, const reco::VertexCollection &primVertices) const;
	...
	double				seedPointMinDist;

};

InclusiveVertexFinderMod::InclusiveVertexFinderMod(const edm::ParameterSet &params) :
	...
	vtxReco(new ConfigurableVertexReconstructor(params.getParameter<edm::ParameterSet>("vertexReco"))),
        ...
        seedPointMinDist(params.getParameter<double>("seedPointMinDist"))
...
bool InclusiveVertexFinderMod::trackFilter(const reco::TrackRef &track) const
{
...
bool InclusiveVertexFinderMod::trackFromPrimVert(const reco::TrackRef &track, const reco::VertexCollection &primVertices) const
{
  for (reco::VertexCollection::const_iterator iPrimVert = primVertices.begin(); iPrimVert != primVertices.end(); ++iPrimVert) {
    double distToPrimVert = abs(iPrimVert->position().z()-primVertices[0].position().z());
    if (iPrimVert->ndof() >= 4 && distToPrimVert < 24 && iPrimVert->trackWeight(track) >= 0.5) return true; /// USE DIFFERENT TRACK WEIGHT??? ALSO IMPOSE REQUIREMENT ON ERROR OF VERTEX???
  }
  
  return false;
}

void InclusiveVertexFinderMod::produce(edm::Event &event, const edm::EventSetup &es)
{
...
// 		  	continue;
	...
	for(std::vector<TracksClusteringFromDisplacedSeed::Cluster>::iterator cluster = clusters.begin();
        ...
        {
                if(cluster->tracks.size() == 0 || cluster->tracks.size() > maxNTracks || cluster->seedPoint.perp() < seedPointMinDist) /// MODIFIED: CLUSTER->SEEDPOINTMINDIST.PERP() < SEEDPOINTMINDIST CUT
		     continue;
...
DEFINE_FWK_MODULE(InclusiveVertexFinderMod);

...
for rhocut in rhoRange :
	for version in range (0, maxRange) :
		for compTag in ["compAll", "compNext", "compIvf", "compRho"] :
			listTag = ["CSVIVFv2-StandardRho25", "CSVIVFv2-StandardRho90", "CSVIVFv2-StandardRho9999"]
			tagselection = "rho"+rhocut+"v"
			if compTag == "compAll" :
				for x in range (0, version+1) :
					listTag.append("CSVIVFv2-NICleanedRho"+rhocut+"v"+str(x))
					tagselection = tagselection+str(x)
			elif compTag == "compNext" and not (version==0 or version==maxRange-1) :
				listTag.append("CSVIVFv2-NICleanedRho"+rhocut+"v"+str(version))
				listTag.append("CSVIVFv2-NICleanedRho"+rhocut+"v"+str(version+1))
				tagselection = tagselection+str(version)+str(version+1)
			elif compTag == "compIvf" and version < 4 :
				listTag.append("CSVIVFv2-NICleanedRho"+rhocut+"v"+str(version))
				listTag.append("CSVIVFv2-NICleanedRho"+rhocut+"v"+str(version+4))
				tagselection = tagselection+str(version)+str(version+4)
			elif compTag == "compRho" and rhocut == "25" :
				tagselection = "comp"
				addColor = 0
				for x in rhoRange :
					tagName = "CSVIVFv2-NICleanedRho"+x+"v"+str(version)
					listTag.append(tagName)
					tagselection = tagselection+"Rho"+x
					mapColor[tagName] = mapColor[tagName] + addColor
					addColor = addColor+1
				tagselection = tagselection+"v"+str(version)
			else : continue
			print "\nTAGSELECTION:", tagselection, listTag, '\n'
			DirName = "Output/RelValProdQCD_Pt_3000_3500_13__CMSSW_7_2_0_pre5-POSTLS172_V3-v1__GEN-SIM-RECO/NIplotFactory_DQM_06_new2/"+tagselection
			c = {}
			valHistos = {}
			refHistos ={}
			Histos = {}
				...
				for h in listHistos :
					...
					for tag in listTag :
						...
						if h.doPerformance :
							...
							if fileRef : refHistos[keyHisto]=graphProducer(plot=h,histos=h_Ref,isVal=False)
						else :    
							valHistos[keyHisto]=histoProducer(plot=h,histos=h_Val,keys=listFlavors,isVal=True)
							if fileRef : refHistos[keyHisto]=histoProducer(plot=h,histos=h_Ref,keys=listFlavors,isVal=False)

        ...
        if h.name=="FlavEffVsBEff_B_discr" :
            for f in ["C","DUSG"] :
                for isVal in [True] :
                    ...
                    #save canvas
                    c[keyHisto] = savePlots(title=titleFlav,dirname=DirName+"/"+b,saveName=h.name+"_"+saveName,listFromats=listFromats,plot=h,Histos=Histos[keyHisto],keyHisto=keyHisto,listLegend=h.listTagger,options=options,legendName=h.legend.replace("FLAV",f))

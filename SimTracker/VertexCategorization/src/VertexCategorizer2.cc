...
VertexCategorizer2::VertexCategorizer2(edm::ParameterSet const & config) :
        // quality_(config),
		HistoryBase(),
        ...
        // beamSpotLabel_( config.getUntrackedParameter<edm::InputTag>("beamSpot") )
{
	...
	// badPull_ = config.getUntrackedParameter<double>("badPull");	// --> store pull value directly and decide later on whether it is "bad"
...
void VertexCategorizer2::newEvent(edm::Event const & event, edm::EventSetup const & setup)
{
	reset();
		...
		for (size_t iDaughter = 0; iDaughter < vCand.numberOfSourceCandidatePtrs(); ++iDaughter)
		{
			...
			for (size_t iTrack = 0; iTrack < trackCollection_->size(); ++iTrack)
			{
				...
				if (&(*trackPtr) == daughterTrack)
					daughterTracks.push_back(trackPtr);
			}
			if ( !daughterTrack)
				std::cout << "Track " << iDaughter << ": " << daughterTrack->pt() << " | " << daughterTracks[daughterTracks.size()-1]->pt() << std::endl;
		}
		std::cout << std::endl;

		newVertex.trackRecoToSim = trackAssociator_->associateRecoToSim(daughterTracks, tpRefVec_, &event, &setup);
...
void VertexCategorizer2::evaluate(edm::ParameterSet const & config, VertexMCInformation & vertInfo)
{
	...
	// for (edm::RefToBaseVector<reco::Track>::const_iterator iTrackRef = daughterTracks.begin(); iTrackRef != daughterTracks.end(); ++iTrackRef)
	// {
...
// VertexCategorizer2 const & VertexCategorizer2::evaluate (reco::VertexBaseRef const & vertex)
// {
...
// VertexCategorizer2 const & VertexCategorizer2::evaluate (TrackingVertexRef const & vertex)
// {
//     // Initializing the category vector
//     reset();
// 
...
// void VertexCategorizer2::simulationInformation()
// {
...
// bool VertexCategorizer2::checkBInHistory(HistoryBase::GenParticleTrail const & genParticleTrail)
// {
...
// bool VertexCategorizer2::checkBInHistory(HistoryBase::SimParticleTrail const & simParticleTrail)
// {
...
// const double VertexCategorizer2::weight(G4::Process processQuery) const
// {
...
// int VertexCategorizer2::vertexInformationDist(HistoryBase const & tracer_)
// {
// 	if (genpvs_.size() == 0){
...
// bool VertexCategorizer2::checkIsBad(reco::TrackBaseRef const & track, TrackingParticleRef const & tpr)
// {
...
// void VertexCategorizer2::qualityInformation(HistoryBase::SimParticleTrail const & spt, reco::TrackBaseRef const & track)
// {
...
int VertexCategorizer2::vertexInformationClust()
{
    ...
    // Loop over the generated particles
    for (
        HistoryBase::SimVertexTrail::reverse_iterator ivertex = simVertexTrail.rbegin();
...
bool VertexCategorizer2::isFinalstateParticle(const HepMC::GenParticle * p)
{
    return !p->end_vertex() && p->status() == 1;
}


bool VertexCategorizer2::isCharged(const HepMC::GenParticle * p)
{
...
// void VertexCategorizer2::genPrimaryVertices(edm::Handle<TrackingVertexCollection> const & TVCollection)
// {
//     genpvs_.clear();
...
// TrackingParticleRef VertexCategorizer2::getSourceTrack(TrackingVertexRef const & evaluatedVertex) const
// {
...
// int VertexCategorizer2::getLongLivedGenParticle(TrackingVertexRef const & tv) const {
...
void VertexCategorizer2::genPrimaryVertices()
{
